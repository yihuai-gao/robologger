...
logger = MobileBaseLogger(
    name="mobile_base_mixed",
    endpoint="tcp://localhost:55561",
    attr={
        "robot_type": "mobile_base",
        "test_mode": "mixed_mode"
    },
    log_state_pose=True,
...
print(f"[{logger.name}] Starting test - mixed mode (state has pose+vel, target has vel only)")
